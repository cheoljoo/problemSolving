...
class Solution:
    def hiking(self, target: List[int] , o : List[List[int]]) -> int:
        ...
        r = self.dijkstra(row,col,edges,(target[0]-1)*col+(target[1]-1))
        
        # self.printMap(row,r)

        # 각 자리를 0부터 가는 것으로 더해줄때의 최소값을 구하면 된다. 0에서 가는 값으로 각기 더해준다.
        for i in range(row):
            r[i] += o[0][i]**2
            r[(col-1)*row+i] += o[row-1][i]**2
        for i in range(1,row-1):
            r[i*col+0] += o[i][0]**2
            r[i*col+col-1] += o[i][col-1]**2
