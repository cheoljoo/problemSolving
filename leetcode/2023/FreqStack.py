...
class FreqStack:
    ...
    def pop(self) -> int:
        if len(self.stack[self.topLevel].keys()) == 0:
            print("ERROR")
            return -1
        print(self.stack[self.topLevel].keys())
        print(len(self.stack[self.topLevel].keys()))
        p = list(self.stack[self.topLevel].keys())[-1]
        self.stack[self.topLevel].pop(p,None)
        if len(self.stack[self.topLevel].keys()) == 0:
            self.topLevel -= 1
        return p
