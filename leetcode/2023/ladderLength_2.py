...
class Solution:
    ...
    def ladderLength(self, beginWord: str, endWord: str, wordList: List[str]) -> int:
        ...
        while q:
            ...
            while q:
                n = q.pop(0)
                if endWord == n:
                    print(" while : ", count , time.time() - start , "-> ", end="") 
                    return lvl
                if n in v:   # it can be possible when it does not weight of edges.  (that means all weight is 1.  we can not get any detours expect direct path.)
                    continue
                v[n] = 1
                for i in range(len(wordList)):
                    b = self.diff(n,wordList[i])
                    if b and wordList[i] not in v: # only one char
                        nextQ.append(wordList[i])
                        count += 1
            q = nextQ
        print(" while : ", count , time.time() - start , "-> ", end="") 
        return 0
    ...
    #     #     pass
    #     print(" make1 : ", len(wordList), time.time() - start , "-> ", end="") 
    #     start = time.time()
    #     for i in range(len(wordList)):
    #         self.mdt[wordList[i]] = set()
    #     # if s not in wordList:
    #     #     for i in range(len(wordList)):
    #     #         b = self.diff(s,wordList[i])
    #     #         if b : # only one char
    #     #             self.mdt[s].add(wordList[i])
    #     q = []
    #     for i in range(len(wordList)):
    #         b = self.diff(s,wordList[i])
    #         if b : # only one char
    #             # self.mdt[s].add(wordList[i])
    #             q.append((s,wordList[i]))
    #     print(" make1-1 : ", len(wordList), time.time() - start , "-> ", end="") 
    #     start = time.time()
    #     # for i in range(len(wordList)):
    #     #     for j in range(i+1,len(wordList)):
    #     #         b = self.diff(wordList[i],wordList[j])
    #     #         if b : # only one char
    #     #             self.mdt[wordList[i]].add(wordList[j])
    #     #             self.mdt[wordList[j]].add(wordList[i])
    #     tcount = 0
    #     while q:
    #         ss,ee = q.pop(0)
    #         self.mdt[ss].add(ee)
    #         self.mdt[ee].add(ss)
    #         for i in range(len(wordList)):
    #             if wordList[i] not in self.mdt[ee]:
    #                 b = self.diff(ee,wordList[i])
    #                 if b : # only one char
    #                     q.append((ee,wordList[i]))
    #                     tcount += 1
    #     print(" make2 : tcount ", tcount,time.time() - start , "-> ", end="") 
...
def run(beginWord , endWord, wordList,expect):
    ...
    print(r, A.diffCount , beginWord , end="")  
    print()
